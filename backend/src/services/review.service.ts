/**
 * Service métier des avis.
 *
 * Règles clés : un seul avis par couple (utilisateur, restaurant) ; toute
 * mutation déclenche `recalculateRating` pour maintenir la note moyenne du
 * restaurant ; les listes d'avis sont mises en cache (TTL court).
 */
...
export async function createReview(
  ...
  // Contrainte d'unicité métier : un utilisateur ne peut noter qu'une fois.
...
export async function replyToReview(id: string, ownerId: string, reply: string) {
  ...
  if (!review) throw new AppError('REVIEW_NOT_FOUND', 'Avis introuvable.', 404);
  // Seul le propriétaire du restaurant visé par l'avis peut y répondre.
